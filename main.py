            ...
            for i in range(psd.matrix.size):
                if i in range(start_val, end_val):
                    for j in range(psd.matrix.size):
                        ...
                        p_coord = np.array(
                            [
                            psd.start[0]*(1-i/8) + psd.end[0]*(i/8),
                            psd.start[1]*(1-i/8) + psd.end[1]*(i/8),
                            psd.min*(1-j/8) + psd.max*(j/8),
                            ]
                        )
                        bez_coord = np.array(
                            [
                            *bezier((i + bez_factor)/8, points),
                            psd.min*(1-j/8) + psd.max*(j/8)
                            ])
    ...
    def map_to_bezier( self, index, points, start):
        ...
        for j in range(self.matrix.size):
            if j in range(start_val, end_val):
                for i in range(self.matrix.size):
                    p = self.matrix.get(i, j)
                    p_coord = np.array( [*plane(i/8, j/8, self.orient), self.z])
                    align_coord = np.array( [*plane(0, j/8, self.orient), self.z])
                    bez_coord =(1-i/8)*align_coord + (i/8) * np.array(
                        [
                        *bezier((j + bez_factor)/8, points),
                        self.z
                        ])
                    new_coord = bez_coord - p_coord
                    # new_coord = new_coord + bez_normal_coord(k, j, factor)
...
def create_stack_list( vmf, t_tex, v_tex):
    ...
    for pre_stack in psd_x_y_sortering_list:
        ...
        td = tDispSide( top_side, top_vertices_returner( solid, z_value ), z_value )
...
def main_func(filepath, t_tex, v_tex):
    ...
    for stack1, stack2 in neighbour_list:
        ...
        mp1 =   line( 3/4, stack1.start, stack1.end )
        mp2 =   line( 1/4, stack2.start, stack2.end )
